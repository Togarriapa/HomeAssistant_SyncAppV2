...
Candidate dependency evidence is now bound to a canonical SHA-256 fingerprint of the exact `RuntimeInventoryInput` used during reference classification. The dependency gate fingerprints the runtime evidence before analysis and proves the same fingerprint again before returning. Downstream consumers can call `verify_candidate_dependency_analysis()` to reject structurally tampered evidence or a different runtime snapshot.
