...
Before any remote mutation, the transport independently re-reads both the authorized original retained commits and the proposed rebuilt commit chain from the staging Git object database. It requires every proposed commit to be the byte-for-byte deterministic parent rewrite of its corresponding authorized commit, requires the original retained ancestry to match the authorization, requires the rebuilt history to remain linear, and requires the oldest retained snapshot to be a new root. A same-tree commit with substituted author, committer, message or other headers is rejected alongside a valid-looking arbitrary SHA or forged `DatabaseHistoryReplacementArtifact`.
...
Only then may the artifact be published. The push targets exactly `refs/heads/database` and uses `--force-with-lease=refs/heads/database:<expected-head>`. A concurrent branch update therefore fails closed. Private Git authentication uses a per-attempt mode-0700 askpass helper and mode-0600 token file inside a temporary staging-Git directory. Git runs through an empty bounded environment with terminal prompts and system/global configuration disabled; the token never enters argv or the repository URL, and the entire authentication directory is removed after success or failure. The transport never writes `main`, `candidate`, `runtime`, `logs`, Home Assistant configuration, or Recorder files.
