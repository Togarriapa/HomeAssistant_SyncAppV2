...
- rescans and re-hashes the entire staged file set before returning success;
...
`verify_candidate_stage()` lets every later validation step re-bind itself to the exact accepted manifest and staged bytes before consuming them. A modified/deleted/inserted file, changed mode, altered manifest or unexpected staging-root entry fails closed.

Stage success still grants no authority to modify Home Assistant.
