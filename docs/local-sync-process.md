...
The processor does not invent a local-change polling cadence. The bounded
[event-driven local synchronization service](local-change-events.md) detects and
debounces stable changes before calling this routine processor. Keeping detection
separate prevents the service loop from becoming an unconditional full-tree
snapshot poll.
