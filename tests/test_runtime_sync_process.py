...
import pytest
from ha_syncapp import runtime_sync_process, runtime_sync_work
...
) -> runtime_sync_process.RuntimeSyncProcessResult:
    return runtime_sync_process.run_runtime_sync_process(
        store,
...
def test_idle_process_does_not_collect_or_consume_other_work(
    tmp_path: Path,
    ...
    monkeypatch.setattr(runtime_sync_process, "collect_core_runtime_bundle", collect)
    try:
...
def test_valid_claim_collects_combined_core_data_then_executes_once(
    ...
    monkeypatch.setattr(runtime_sync_process, "collect_core_runtime_bundle", collect)
    monkeypatch.setattr(runtime_sync_process, "execute_claimed_runtime_sync_work", execute)
    try:
        result = _run(store, tmp_path)
    finally:
        store.__exit__(None, None, None)
...
def test_normal_process_does_not_recover_interrupted_work(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    store = _store(tmp_path)
...
def test_execution_error_is_sanitized(
    ...
    monkeypatch.setattr(runtime_sync_process, "execute_claimed_runtime_sync_work", fail)
    try:
        with pytest.raises(runtime_sync_process.RuntimeSyncProcessError) as caught:
...
def test_unopened_state_store_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(runtime_sync_process.RuntimeSyncProcessError, match="failed closed"):
        _run(store, tmp_path)
