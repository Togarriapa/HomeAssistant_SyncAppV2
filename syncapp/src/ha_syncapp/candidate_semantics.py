...
def validate_candidate_semantics(
    ...
    try:
        result = _verify_inputs(static, integrity, stage, dependencies, impact, risk, runtime, version)
        _check_supported_tree(stage)
        # Explicit /tmp avoids an inherited TMPDIR pointing into the live config or app state.
        with tempfile.TemporaryDirectory(
            prefix="syncapp-validator-", dir="/tmp"  # nosec B108
        ) as directory:
        ...
        raise CandidateSemanticError("semantic validation evidence could not be established") from None
...
def _verify_inputs(
    ...
    verify_candidate_static_validation(static, integrity, stage, dependencies, impact, risk, runtime)
    ...
    if version.version != BUNDLED_CORE_VERSION:
        raise CandidateSemanticError("running Core version does not match bundled validator version")
    return CandidateSemanticValidation(
        static.target, static.repository_id, static.baseline_sha, static.candidate_sha,
        static.stage_manifest_sha256, static.runtime_sha256, static.risk_level, version.version,
    )
...
def _check_supported_tree(stage: CandidateStage) -> None:
    ...
    for entry in stage.entries:
        ...
        try:
            documents = list(yaml.compose_all(_read_bound_bytes(stage, entry), Loader=yaml.SafeLoader))
...
def _check_includes(root: Node, path: PurePosixPath) -> None:
    ...
    while pending:
        ...
        if node.tag.startswith("!include"):
            ...
            if (
                not value or target.is_absolute() or ".." in target.parts
...
def _run_validator(config: Path, version: str) -> None:
    ...
    command = [
        _CORE_PYTHON, "-I", "-B", str(Path(__file__).with_name("_validator_child.py")),
        str(config), version, nonce,
    ]
    environment = {
        "PATH": "/usr/local/bin:/usr/bin:/bin", "LANG": "C.UTF-8",
        "TMPDIR": str(config.parent), "XDG_CACHE_HOME": str(config.parent),
    ...
    with tempfile.TemporaryFile() as receipt:
        with subprocess.Popen(  # nosec B603
            command, stdin=subprocess.DEVNULL, stdout=receipt, stderr=subprocess.DEVNULL,
            cwd="/", env=environment, close_fds=True, start_new_session=True,
        ) as process:
            try:
                ...
                try:
                    ...
                    pass
                process.wait()
                raise CandidateSemanticError("semantic validator timed out or was interrupted") from None
        receipt.seek(0)
