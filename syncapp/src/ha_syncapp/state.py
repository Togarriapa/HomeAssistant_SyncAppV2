...
class StateStore:
    ...
    @staticmethod
    def _create_live_apply_progress_table(db: sqlite3.Connection) -> None:
        db.execute(
            "CREATE TABLE live_apply_progress ("
            ...
            "operation_path_sha256 TEXT NOT NULL, "
            "phase TEXT NOT NULL CHECK (phase IN ("
            "'mutation_started','mutation_verified','blocked')), "
            "updated_at TEXT NOT NULL, record_sha256 TEXT NOT NULL, "
            "PRIMARY KEY (deployment_id, operation_index))"
        )

    @staticmethod
    def _create_live_apply_recovery_tables(db: sqlite3.Connection) -> None:
        db.execute(
            "CREATE TABLE IF NOT EXISTS live_apply_mutation_guard ("
            "deployment_id TEXT NOT NULL, operation_index INTEGER NOT NULL "
            "CHECK (operation_index >= 0), intent_record_sha256 TEXT NOT NULL, "
            "operations_sha256 TEXT NOT NULL, operation_path_sha256 TEXT NOT NULL, "
            "root_device INTEGER NOT NULL CHECK (root_device >= 0), "
            "root_inode INTEGER NOT NULL CHECK (root_inode > 0), "
            "parent_identities_json TEXT NOT NULL, recorded_at TEXT NOT NULL, "
            "record_sha256 TEXT NOT NULL, PRIMARY KEY (deployment_id, operation_index))"
        )
        db.execute(
            "CREATE TABLE IF NOT EXISTS live_apply_reconciliation ("
            "deployment_id TEXT NOT NULL, operation_index INTEGER NOT NULL "
            "CHECK (operation_index >= 0), outcome TEXT NOT NULL "
            "CHECK (outcome IN ('applied','not_applied','ambiguous')), "
            "recorded_at TEXT NOT NULL, record_sha256 TEXT NOT NULL, "
            "PRIMARY KEY (deployment_id, operation_index))"
        )
    ...
    @staticmethod
    def _create_core_restart_attempt_table(db: sqlite3.Connection) -> None:
        db.execute(
            "CREATE TABLE IF NOT EXISTS core_restart_attempt ("
            "deployment_id TEXT PRIMARY KEY NOT NULL, "
            "authorization_record_sha256 TEXT NOT NULL, "
            "phase TEXT NOT NULL CHECK (phase IN ('request_started','request_acknowledged')), "
            "started_at TEXT NOT NULL, updated_at TEXT NOT NULL, record_sha256 TEXT NOT NULL)"
        )

    @staticmethod
    def _create_core_health_observation_table(db: sqlite3.Connection) -> None:
        db.execute(
            "CREATE TABLE IF NOT EXISTS core_health_observation ("
            "deployment_id TEXT PRIMARY KEY NOT NULL, "
            "restart_attempt_sha256 TEXT NOT NULL, observed_at TEXT NOT NULL, "
    ...
    @staticmethod
    def _create_automation_script_observation_table(db: sqlite3.Connection) -> None:
        db.execute(
            "CREATE TABLE IF NOT EXISTS automation_script_observation ("
            "deployment_id TEXT PRIMARY KEY NOT NULL, "
            "entity_state_observation_sha256 TEXT NOT NULL, target_sha256 TEXT NOT NULL, "
            "observed_at TEXT NOT NULL, expected_count INTEGER NOT NULL "
            "CHECK (expected_count >= 0), loaded_count INTEGER NOT NULL "
            "CHECK (loaded_count >= 0 AND loaded_count <= expected_count), "
            "failed_count INTEGER NOT NULL CHECK (failed_count >= 0 AND "
