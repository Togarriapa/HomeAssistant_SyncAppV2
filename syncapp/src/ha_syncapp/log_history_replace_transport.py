...
def build_log_history_replacement(
    *,
    ...
    """Rebuild retained logs commits while severing only authorized pruned ancestry."""
...
def replace_logs_history(
    *,
    ...
    """Publish only a validated builder-produced logs replacement with an exact lease."""
...
def _validate_authorization_boundary(
    ...
    if (
        ...
        or any(_COMMIT_SHA.fullmatch(sha) is None for sha in authorization.retained_shas)
        or any(_COMMIT_SHA.fullmatch(sha) is None for sha in authorization.pruned_shas)
        or any(
            len(sha) != len(authorization.expected_head_sha)
            for sha in (*authorization.retained_shas, *authorization.pruned_shas)
        )
    ):
        raise LogHistoryReplacementTransportError("logs replacement authorization is invalid")
...
def _read_commit(
    *,
    ...
    try:
        ...
        raise LogHistoryReplacementTransportError("logs retained history could not be read") from None
    if (
...
def _write_commit_object(
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="syncapp-log-history-", dir=repository / ".git") as temporary:
        ...
        raise LogHistoryReplacementTransportError("logs retained history could not be rebuilt") from None
    if (
        not isinstance(result, subprocess.CompletedProcess)
        or result.returncode != 0
        or not isinstance(result.stdout, str)
    ...
    if _COMMIT_SHA.fullmatch(replacement_sha) is None or len(replacement_sha) != expected_sha_length:
        raise LogHistoryReplacementTransportError("logs rebuilt history is invalid")
...
def _validate_repository_identity(authorization: LogHistoryReplacementAuthorization) -> None:
...
def _validate_repository_path(repository: Path) -> Path:
    ...
    try:
        ...
        raise LogHistoryReplacementTransportError("logs replacement repository path is invalid") from None
    if not stat.S_ISDIR(metadata.st_mode) or repository.is_symlink() or not (resolved / ".git").is_dir():
        raise LogHistoryReplacementTransportError("logs replacement repository path is invalid")
    return resolved
